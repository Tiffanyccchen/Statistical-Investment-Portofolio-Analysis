...
X<-cbind(tw50[,2],tw2049[,2],tw2383[,2],tw2439[,2])
X1<-cbind(tw50[,2],a1,a2,a)
a1<-mean(tw50[,2])
a2<-sd(tw50[,2])
b1<-mean(tw2049[,2])
b2<-sd(tw2049[,2])
...
row.names(mean.sd)<-c("tw50","上銀","台光電","美律")
...
sum(omegafinal)
v_tangency_fixed<-c(omegafinal,1-sum(omegafinal))
...
VAR<-c()
for(i in 1:10){
VAR[i]<--(0.004538+qnorm(a[i])*0.0226)*100000000
}
VAR
